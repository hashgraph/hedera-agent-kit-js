...
| `CREATE_NON_FUNGIBLE_TOKEN_TOOL`              | Creates a non-fungible token (NFT) on Hedera                  | Provide token name and symbol (required). Optionally set supply type ("finite" or "infinite", defaults to "finite"), max supply (only applicable for finite supply, defaults to 100), treasury account ID (defaults to operator), and supply key. If maxSupply is provided, supply type will be set to finite. Supports scheduled transactions.                                                                                                                                                                                                                                                        |
