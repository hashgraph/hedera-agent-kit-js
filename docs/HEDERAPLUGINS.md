...
| `APPROVE_TOKEN_ALLOWANCE_TOOL`  | Approve fungible token spending allowances                 | Provide optional owner account ID (defaults to operator if omitted), the spender account ID (required), an array of token allowances each with tokenId and amount (positive integer), and an optional transaction memo. Returns transaction ID and receipt/status. |
