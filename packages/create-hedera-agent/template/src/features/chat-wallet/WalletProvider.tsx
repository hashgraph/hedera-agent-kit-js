...
// Locally-declared shape of a tool summary so this feature stays decoupled
// from the rest of the app. Any caller that passes a compatible
// `{ title, fields }` object satisfies the contract.
...
export type PendingApproval = {
  ...
  // Callbacks supplied by the caller at enqueue time. The drawer calls them
  // without needing direct access to the chat substrate's
  // ChatToolActionsProvider, which is mounted lower in the tree.
  onApprove: (signedBytes: string) => Promise<void>;
  onReject: () => Promise<void>;
};

export type WalletSigner = (unsignedBytesBase64: string) => Promise<string>;
...
export type WalletContextValue = {
  ...
  clearApproval: (toolCallId: string) => void;
  signTransactionBytes: WalletSigner;
};
...
export function ChatWalletProvider({
  ...
  const enqueueApproval = React.useCallback((approval: PendingApproval) => {
  ...
  const clearApproval = React.useCallback((toolCallId: string) => {
    setPendingApprovals((current) => {
