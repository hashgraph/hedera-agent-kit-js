...
import type { Summary } from "@/features/chat-hedera/utils/transaction-summaries";

export type ChatHederaTransactionCardActionsProps = {
  toolName: string;
  toolCallId: string;
  input: unknown;
  summary: Summary;
  // Absent on legacy chats persisted before the wallet flow shipped. The
  // block falls back to a discard-only affordance in that case.
...
export function ChatHederaTransactionCardActions({
  ...
  summary,
  unsignedBytes,
}: ChatHederaTransactionCardActionsProps) {
  ...
  React.useEffect(() => {
    ...
    const pushResult = (envelope: ToolEnvelope) =>
      addToolResult({
        tool: toolName,
        toolCallId,
    ...
    const onApprove = async (signedBytes: string) => {
