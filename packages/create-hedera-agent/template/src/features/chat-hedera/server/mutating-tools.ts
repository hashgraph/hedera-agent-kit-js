...
export function getMutatingToolMethods(plugins: Plugin[]): Set<string> {
...
export function isMutatingTool(toolName: string, plugins: Plugin[]): boolean {
  return getMutatingToolMethods(plugins).has(toolName);
}

function isQueryPlugin(plugin: Plugin): boolean {
