...
export type AwaitingApprovalPayload = {
  raw: {
    ...
    // Base64-encoded transaction bytes the user signs off-server. Always
    // present for native human-mode envelopes produced by the current code;
    // optional in the type for legacy chats persisted before this change.
...
function wrapMutatingToolsForApproval(
...
function parseToolOutputForModel(output: unknown): Record<string, unknown> | null {
  if (typeof output !== "string") {
    if (output && typeof output === "object" && !Array.isArray(output)) {
      return stripBytes(output as Record<string, unknown>);
    }
    return null;
  }
  try {
    ...
    return stripBytes(parsed as Record<string, unknown>);
  } catch {
    return null;
  }
}

function stripBytes(envelope: Record<string, unknown>): Record<string, unknown> {
