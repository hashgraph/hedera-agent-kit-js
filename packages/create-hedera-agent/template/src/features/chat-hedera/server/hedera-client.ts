...
import { HEDERA_NETWORK, type HederaNetwork } from "@/features/chat-hedera/utils/network";
import { parseOperatorKey } from "@/features/chat-hedera/utils/operator-key";

export type HederaEnv = {
  operatorId: string;
  operatorKey: string;
  // Derived from `operatorKey`; the server needs it to set the signing key on
  // the human-mode client when freezing transactions for the browser wallet.
  operatorPublicKey: string;
};

export function readEnv(): HederaEnv {
...
export function createHederaClient(env: HederaEnv = readEnv()): Client {
...
export function createReturnBytesHederaClient(env: HederaEnv = readEnv()): Client {
  const client = clientForNetwork(HEDERA_NETWORK);
...
export function createSubmitClient(): Client {
