...
function plugin(name: string, methods: string[]): Plugin {
  return {
    name,
    version: "1.0.0",
    tools: () =>
      methods.map((method) => ({
        ...
        name: method,
        description: "",
        // The tool factory's return type expects more than this, but only
        // `method` is read by getMutatingToolMethods — cast to keep the test
        // focused on the bucketing logic.
        parameters: undefined as unknown as Plugin["tools"] extends never
          ? never
          : never,
        execute: async () => undefined,
      })) as unknown as ReturnType<Plugin["tools"]>,
  };
}
