...
// Convenience re-exports keep call sites (cards, tests) terse without each
// importing the substrate types directly.
export type Summary = ToolSummary;
export type SummaryField = ToolSummaryField;
export type SummaryFormatter = ToolSummarizer;
export type TransactionSummariesRegistry = Record<string, SummaryFormatter>;

const transferHbarSummary: SummaryFormatter = (input) => {
...
const createFungibleTokenSummary: SummaryFormatter = (input) => {
  const params = asRecord(input);
  const name = asString(params.tokenName) ?? "(unnamed)";
  const symbol = asString(params.tokenSymbol);
  const fields: SummaryField[] = [{ label: "Name", value: name }];
...
const mintFungibleTokenSummary: SummaryFormatter = (input) => {
  const params = asRecord(input);
...
const associateTokenSummary: SummaryFormatter = (input) => {
  const params = asRecord(input);
...
const submitTopicMessageSummary: SummaryFormatter = (input) => {
  const params = asRecord(input);
...
const createTopicSummary: SummaryFormatter = (input) => {
  const params = asRecord(input);
...
const transferNonFungibleTokenSummary: SummaryFormatter = (input) => {
  const params = asRecord(input);
  const tokenId = asString(params.tokenId) ?? "(unknown)";
...
export const transactionSummaries: TransactionSummariesRegistry = {
...
// uses internally.
export function summarize(toolName: string, input: unknown): Summary {
...
function defaultSummary(toolName: string, input: unknown): Summary {
