...
import type { ChatIndexEntry, StoredChat } from "./types";

export type { ChatIndexEntry, StoredChat } from "./types";
export type { CreateChatOptions } from "./chat-store";
...
function scanForStaleEntries(): void {
...
export function loadChatIndex(): ChatIndexEntry[] {
  scanForStaleEntries();
  const adapter = getBrowserStorage();
  if (!adapter) return [];
...
export function createChat(options: CreateChatOptions = {}): StoredChat {
  scanForStaleEntries();
  const adapter = getBrowserStorage();
  if (!adapter) {
...
export function renameChat(id: string, newTitle: string): StoredChat | null {
  const adapter = getBrowserStorage();
  if (!adapter) return null;
