...
import type { StorageAdapter, StoredChat } from "@/features/chat/state/types";

type AdapterOptions = {
  quotaBytes?: number;
};

function createMemoryAdapter(options: AdapterOptions = {}): StorageAdapter & {
  size(): number;
...
function messageOfText(role: "user" | "assistant", text: string): ChatMessage {
  return {
    id: `msg-${role}-${text}`,
    role,
