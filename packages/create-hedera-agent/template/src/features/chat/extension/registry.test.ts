...
import type { ChatExtension, ToolSummarizer } from "./types";
...
function ext(overrides: Partial<ChatExtension> & { id: string }): ChatExtension {
  return { ...overrides };
}
...
describe("mergeExtensions", () => {
  ...
  describe("per-tool slot collisions", () => {
    it("should warn and last-write-win when two extensions register a summarizer for the same tool", () => {
      const first: ToolSummarizer = () => ({ title: "first", fields: [] });
      const second: ToolSummarizer = () => ({ title: "second", fields: [] });
  ...
  describe("request body aggregation", () => {
    ...
    it("should skip non-object request-body contributions", () => {
      const a = ext({
        ...
        getRequestBody: () => null as unknown as Record<string, unknown>,
      });
