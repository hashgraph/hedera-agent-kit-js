...
import type { ActivityInput, ActivityViewModel } from "@/features/chat/utils/agent-activity";
...
export type ChatActivityProps = {
  input: ActivityInput;
  // Read from the server-derived toolkit set so the timeline row mapper can
  // minimize the expanded panel for tools whose inline card already carries
...
export function ChatActivity({ input, mutatingToolMethods }: ChatActivityProps) {
...
function useStoppedHold(view: ActivityViewModel): boolean {
  const prevKindRef = React.useRef<ActivityViewModel["kind"]>(view.kind);
...
function syntheticAgentErrorRow(errorMessage: string): TimelineRowViewModel {
  return {
