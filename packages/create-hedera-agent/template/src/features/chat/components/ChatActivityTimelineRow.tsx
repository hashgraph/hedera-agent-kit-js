...
export type ChatActivityTimelineRowProps = {
  row: TimelineRowViewModel;
  // Underlying tool part for `row`. Threaded through so the substrate can call
  // an extension-registered `row` renderer with canonical `ToolPartProps`.
  // Absent for synthetic rows (e.g. the whole-turn agent-error row).
  part?: ChatToolPart;
};
...
function DefaultPanel({ row }: { row: TimelineRowViewModel }) {
  return (
    <>
...
function toolPartPropsFromPart(part: ChatToolPart): ToolPartProps {
  return {
