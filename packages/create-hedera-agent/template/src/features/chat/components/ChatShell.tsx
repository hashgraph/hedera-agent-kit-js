...
export type ChatShellProps = {
  chatId: string;
  // Method names whose tools mutate Hedera state. Computed in the server
  // component above and threaded through as an array (Sets don't cross the
  // RSC boundary directly); the chat surface converts it back into a Set.
  mutatingToolMethods: string[];
  // App-composed header controls (mode toggle, wallet button, etc.). The
  // shell forwards them to ChatHeader without inspecting their shape.
  headerSlots?: React.ReactNode;
};

type HydrationState =
  | { status: "loading" }
  | { status: "ready"; chat: StoredChat }
  | { status: "missing" };
...
export function ChatShell({
  ...
  const [state, setState] = React.useState<HydrationState>({ status: "loading" });
