...
export type ActivityInput =
  | { kind: "pending"; status: ChatStatus }
  | {
      kind: "message";
      message: ChatMessage;
      isLive: boolean;
...
export type ActivityViewModel =
  | { kind: "hidden" }
  | { kind: "wait" }
  | { kind: "progress"; label: string; stepCount: number }
  | { kind: "resting"; stepCount: number; cancelled?: boolean }
  | { kind: "resting-failed"; stepCount: number; errorMessage: string };

export type DeriveActivityOptions = {
  // Per-tool summarizer registry, typically threaded in from the merged
  // ChatExtension registry. The substrate falls back to a generic humanized
  // tool name for any tool the registry doesn't cover.
...
function countToolParts(message: ChatMessage): number {
...
export function hasUnfinishedParts(message: ChatMessage): boolean {
...
function resolveProgressLabel(
  message: ChatMessage,
  summarizers?: Readonly<Record<string, ToolSummarizer>>,
): string {
