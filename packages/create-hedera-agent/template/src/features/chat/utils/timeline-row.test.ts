...
function row(input: Parameters<typeof mapTimelineRow>[0]) {
  return mapTimelineRow({ toolSummarizers: fixtureToolSummarizers, ...input });
}

function rows(
  message: ChatMessage,
  options: Omit<MapTimelineRowsOptions, "toolSummarizers"> = {},
) {
...
// toolName, toolCallId.
function toolPart(options: ToolPartOptions = {}): ChatToolPart {
  const {
...
function envelope(raw: Record<string, unknown>, humanMessage?: string): string {
