...
- submitKey (boolean|string, optional): New submit key. Pass boolean true to use your key/operator key, or provide a public key string.
...
Examples:
- If the user asks for "my key", "my public key", or "operator key" → set the field to \`true\`.
- If the user does not mention the key → do not set the field.
- If the user provides a public key string → set the field to the provided public key string.
