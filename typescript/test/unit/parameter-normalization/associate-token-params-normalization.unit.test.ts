...
describe('HederaParameterNormaliser.normaliseAssociateTokenParams', () => {
  ...
  const client = Client.forNetwork({}).setOperator(
    AccountId.fromString('0.0.1111'),
  ...
  it('should use provided accountId and pass tokenIds through', () => {
    ...
    const result = HederaParameterNormaliser.normaliseAssociateTokenParams(params, context, client);
