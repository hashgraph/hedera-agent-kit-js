  ...
  app.post('/mcp', async (req, res) => {
    ...
    try {
      ...
      if (sessionId && transports[sessionId]) {
        ...
        const headerAccountId = req.headers['x-hedera-account-id'] as string | undefined;
        ...
        if (headerAccountId) {
          ...
          try {
            if (options.ledgerId) {
              const mirrorNodeService = getMirrornodeService(undefined, options.ledgerId);
              const accountData = await mirrorNodeService.getAccount(headerAccountId);
