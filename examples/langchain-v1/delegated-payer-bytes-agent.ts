...
async function bootstrap(): Promise<void> {
  ...
  const agent = createAgent({
    ...
    tools: tools,
    systemPrompt: 'You are a helpful Hedera assistant. Prepare transactions on behalf of the user and return unsigned bytes for them to sign.',
    checkpointer: new MemorySaver(),
  });

  console.log('Hedera Agent — Delegated Payer (CUSTOM_RETURN_BYTES, LangGraph/LangChain-v1) — type "exit" to quit');
  ...
  while (true) {
    ...
    try {
      ...
      const parsedTools = responseParser.parseNewToolMessages(response);
      const toolCall = parsedTools[0];
